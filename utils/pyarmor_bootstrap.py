...
def ensure_pyarmor_runtime_on_path():
    ...
    if rt_root is None:
        logger.debug("Dev mode? No 'runtimes' folder found for pyarmor.")
        return False
    ...
    if (exact_base / "pyarmor_runtime_000000").is_dir():
        ...
        logger.debug("Using pyarmor runtime path: {exact_base}")
        return True
    ...
    if candidates:
        logger.debug(f"Using pyarmor runtime path candidate: {candidates[0]}")
